...
describe(projectName, () => {
  ...
  test('The tickets manager load tickets from 8080 and show them on page with labels', async () => {
    const getAllTicketsMock = await nock('http://localhost:3000/', {
      allowUnmocked: true,
    })
      .get('/api/tickets')
      .query(() => true)
    ...
    let firstLabelValue = await (
      await firstLabel.getProperty('innerText')
    ).jsonValue();
  ...
  test('The user can filter tickets by typing on input with id - searchInput', async () => {
    const filterText = 'h';
    const getFilteredTicketsMock = await nock('http://localhost:3000/', {
      allowUnmocked: true,
    })
      .get('/api/tickets')
      .query(({ searchText }) => {
  ...
  test('Can hide tickets (client-side-only), show counter of hidden tickets', async () => {
    ...
    let currentCounter = await (
      await hideTicketsCounter.getProperty('innerText')
    ).jsonValue();
